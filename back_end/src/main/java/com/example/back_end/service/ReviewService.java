...
public class ReviewService {
    ...
    public ReviewResponse createReview(Long userId, CreateReviewRequest request ) {
        ...
        if (reviewRepository.findByUserIdAndCourseId(userId,request.getCourseId()).isPresent()) {

            throw new RuntimeException("You have already reviewed this course");
        }

        User user = userRepository.findById(userId).orElseThrow(() -> new RuntimeException("LỖI: Không tìm thấy người dùng với ID = " + userId));

        Course course = courseRepository.findById(request.getCourseId()).orElseThrow(() -> new RuntimeException("LỖI: Không tìm thấy khóa học với ID = " + request.getCourseId()));
